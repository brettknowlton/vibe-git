...
/*
 * Editing a description is a WRITE against a public artifact, so it goes through
 * ghWrite like create() does. An empty body is legitimate here — it clears the
 * description — so `required: false` and we pass the empty string through rather
 * than dropping the flag, which would leave the old text in place.
 */
async function edit(dir, { number, body }) {
  const n = posInt(number, 'pull request number');
  const b = text(body, 'description', 60000, { required: false }) || '';
  const r = await ghWrite(dir, ['pr', 'edit', String(n), '--body', b], {
    label: 'edit pull request description',
  });
  return {
    ok: true, dryRun: !!r.dryRun, number: n,
    message: (r.dryRun ? 'Would update' : 'Updated') + ' the description for #' + n,
  };
}

module.exports = { list, forBranch, view, create, edit };
