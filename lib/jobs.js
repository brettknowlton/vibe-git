...
class Jobs {
  ...
  start(id, kind, label) {
    ...
    const signal = {
      ...
      // What it is doing RIGHT NOW, as opposed to how far through it is. A classification
      // run has a fraction; a chat turn has "reading lib/search.js" and no denominator.
      step: '',
      startedAt: Date.now(),
    };
  ...
  active() {
    return [...this.map.values()].map(s => ({
      id: s.id, kind: s.kind, label: s.label, step: s.step,
      done: s.done, total: s.total, cancelled: s.cancelled,
      seconds: Math.round((Date.now() - s.startedAt) / 1000),
    }));
  }

  /* Progress is written by whoever is doing the work and read by the status poll. Both are
   * best-effort: a missing signal means the job was cancelled, which is not an error. */
  progress(signal, patch) {
    if (!signal) return;
