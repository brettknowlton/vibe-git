...
const { isCancel, CancelError } = require('./jobs');
 ...
 *
 * A local model that produces unparseable JSON has usually done one specific thing — run out
 * of generation room mid-object, or wrapped the answer in a sentence — and it fixes both when
 * shown its own output and told what was wrong. Failing instead throws away everything the
 * run cost: classification of forty issues, or a plan over a whole tracker, discarded for a
 * missing brace. The retry is bounded at one and is skipped for cancels, so a genuinely
 * broken endpoint still fails fast rather than doubling every timeout.
 */
async function chat(cfg, messages, schema, signal) {
  const spec = await providers.resolve(cfg);
  const ask = (msgs) => spec.chat(cfg, msgs, { schema, signal });
  ...
  const complaint = content
    ? 'Your previous reply was not valid JSON. Reply again with the SAME content as a single ' +
      'valid JSON object matching the schema, and nothing else — no prose, no code fence. ' +
      'If the answer was cut off, shorten the text fields so the object closes.'
    : 'Your previous reply was empty. Answer now with a single valid JSON object matching the schema.';
  const repaired = await ask(messages.concat([
    { role: 'assistant', content: clip(content, 4000) },
    { role: 'user', content: complaint },
  ]));
  const second = (repaired && repaired.content) || '';
  const fixed = second ? parseJson(second) : undefined;
  if (fixed !== undefined) return fixed;

  if (!content && !second) throw new Error('Model returned an empty response');
  throw new Error('Model did not return valid JSON: ' + String(second || content).slice(0, 160));
}
...
async function classify(cfg, { issues, milestones, labels, corpus, allIssues }, signal) {
  ...
  const sys = [
    ...
    /* The only issue numbers this pass sees are the ones written inside the body it is
       reading, and a tracker's bodies are full of references to work that has since
       shipped. Every one of those became a proposal that validation threw away. */
    '- The numbers in an issue body are REFERENCES, not blockers. The body was written once',
    '  and most of what it points at is already CLOSED — finished work blocks nothing, and',
    '  nothing here tells you which is which.',
    '- So return [] unless the issue plainly still needs something that plainly has not',
    '  happened yet. A satisfied dependency is worse than no dependency: it asks the reader',
    '  to record a constraint that stopped applying the day the blocker was closed.',
...
/*
 * What the reader has already told the plan by hiding parts of it.
 *
 * Muting is the only feedback signal this app collects that is about the ORDER rather than
 * about the work, and it is the one thing a regenerated plan otherwise forgets instantly:
 * push an entry down, regenerate, and a model with no memory of the correction puts it
 * straight back at the top. Two lists, phrased as instructions rather than as data, because a
 * heading the model reads as background is a heading it averages away.
 *
 * Dependencies are stated as an absolute prohibition and are ALSO filtered after the fact by
 * the caller. The prompt is the polite version; the filter is the one that has to hold.
 */
function feedbackBlock({ hiddenItems, ignoredDeps }) {
  const lines = [];
  const hidden = (hiddenItems || []).map(item => {
    const label = clip(item && item.label, 160).replace(/\s+/g, ' ').trim();
    const ns = (item && Array.isArray(item.numbers) ? item.numbers : []).filter(Number.isInteger);
    if (ns.length) return `- ${ns.map(n => '#' + n).join(', ')}${label ? ' — ' + label : ''}`;
    return label ? `- proposed issue "${label}"` : '';
  }).filter(Boolean).slice(0, 40);
  if (hidden.length) {
    lines.push(
      'THE USER HAS HIDDEN THESE ENTRIES FROM THE CURRENT PLAN. They judged the previous',
      'ordering wrong about them, so do not restore them near the top:',
      ...hidden,
      '- Rank them below everything the user has not hidden, or leave them out entirely.',
      '- This is about POSITION, not validity. Do not re-argue for them in "why".',
      '');
  }
  const rejected = (ignoredDeps || [])
    .filter(dep => dep && Number.isInteger(Number(dep.blocked)) && Number.isInteger(Number(dep.blockedBy)))
    .slice(0, 60)
    .map(dep => `- #${Number(dep.blocked)} does NOT wait on #${Number(dep.blockedBy)}`);
  if (rejected.length) {
    lines.push(
      'DEPENDENCY EDGES THE USER HAS REJECTED. These were proposed before and refused:',
      ...rejected,
      '- Never propose any of these again, in either direction.',
      '');
  }
  return lines.join('\n');
}

async function planInsights(cfg, { issues, milestones, labels, planText, count, gapCount, wantDeps, depCount, scope, currentPlan, changed, fileMap, hiddenItems, ignoredDeps }, signal) {
  if (!cfg.model) bad('Pick a model first');
  ...
  const sys = [
    ...
    wantDeps === false
      ...
      : [
        ...
        '- Never both directions between one pair.',
        /*
         * Where the closed blockers actually come from.
         *
         * Withholding closed numbers from the ALREADY DONE block is necessary and not
         * sufficient: bodies reference issues that shipped months ago, so an open issue whose
         * body says "needs the chest art #102" carries a closed number INTO the open list.
         * That is the real source — the last plan for this tracker proposed exactly that edge.
         * "Only numbers from the OPEN ISSUES list" reads as permission when the number is
         * sitting right there inside it, so the rule has to name the line, not the list.
         */
        '- A usable number is one that BEGINS an entry in the OPEN ISSUES list, on its own line.',
        '  Numbers written inside an issue body are references, not candidates: the body was',
        '  written once and the work it points at has often long since shipped. If a number is',
        '  not at the start of one of those lines, you may not use it.',
        '- A CLOSED issue is finished work and blocks NOTHING. Never name one as a blocker, and',
        '  never mention one in `why`. If the only thing an issue was waiting on is already done,',
        '  it has no dependency — say nothing about it rather than recording a satisfied one.',
    ...
    '',
    feedbackBlock({ hiddenItems, ignoredDeps }),
    'MILESTONES:',
    milestoneBlock(milestones),
    '',
    'ALLOWED LABELS: ' + JSON.stringify(labelNames),
   ...
   *
   * Their NUMBERS are withheld for the same reason the instructions say not to cite them: a
   * closed number in the prompt is one the model will eventually use as a blocker, and no
   * amount of telling it not to beats not having it. Nothing downstream needs them —
   * recognising "this is already done" is a title-matching job.
   */
  ...
  const closedBlock = closed.length
    ? 'ALREADY DONE — closed issues, titles only and deliberately without their numbers.\n' +
      'They exist so you do not propose missing work that has already been finished. You\n' +
      'cannot rank them and you cannot make anything wait on them:\n' +
      clip(closed.map(i => `- ${i.t}`).join('\n'), closedBudget)
    : '';
...
module.exports = {
  ...
  resolveConflict,
  embed, cosine, topK, issueText, unload, collectNominations, parseJson, pool, feedbackBlock,
