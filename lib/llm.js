...
function post(endpoint, pathname, payload, timeoutMs) {
...
async function embed(cfg, texts) {
  ...
  try {
    const r = await post(cfg.endpoint || DEFAULT_ENDPOINT, '/api/embed',
      { model, input }, cfg.timeoutMs || 300000);
  ...
  for (const t of input) {
    const r = await post(cfg.endpoint || DEFAULT_ENDPOINT, '/api/embeddings',
      { model, prompt: t }, cfg.timeoutMs || 300000);
...
async function chat(cfg, messages, schema) {
...
const CLASSIFY_SCHEMA = {
  ...
  required: ['milestone', 'labels', 'confidence', 'reason'],
};
...
async function classify(cfg, { issues, milestones, labels, corpus }, signal) {
  ...
  const sys = [
    'You triage GitHub issues into project milestones.',
  ...
  const results = await pool(issues, cfg.concurrency || 2, async (i) => {
    ...
    return {
      ...
      milestone,
      milestoneRejected: milestone ? null : g.milestone,
      labels: keep,
  ...
  return results.map((r, idx) => r && !r.error ? r
    : { number: issues[idx].n, title: issues[idx].t, error: (r && r.error) || 'failed' });
}
...
async function suggest(cfg, { issues, milestones, labels, planText, count, corpusVecs }) {
  if (!cfg.model) bad('Pick a model first');
  ...
  if (cfg.embedModel && Array.isArray(corpusVecs) && corpusVecs.length) {
    ...
    try {
      const vecs = await embed(cfg, proposed);
    ...
    } catch { /* fall back to the token test below */ }
  }
 ...
 */
async function suggestMilestones(cfg, { orphans, milestones, planText }) {
  if (!cfg.model) bad('Pick a model first');
 ...
 */
async function planInsights(cfg, { issues, milestones, labels, planText, count, gapCount }) {
  if (!cfg.model) bad('Pick a model first');
  const open = (issues || []).filter(i => i.st === 'OPEN');
  if (!open.length && !planText && !(milestones || []).length) return { ranked: [], gaps: [] };
  const want = Math.max(3, Math.min(count || 10, 20));
  ...
  const sys = [
    ...
    '- Only use issue numbers that appear in the list given.',
    '',
  ...
  const lines = (issues || []).map(i => {
    ...
    const body = clip(i.body, 300).replace(/\s+/g, ' ').trim();
    return `#${i.n} [${i.st}] (${i.ms || 'no milestone'})${box}${refs} ${i.t}` +
      (body ? `\n    ${body}` : '');
  });

  const user = [
    planText ? 'PROJECT CONTEXT:\n' + contextClip(planText, 8000) + '\n' : '',
    'EXISTING ISSUES (open issues may be ranked; closed issues prevent duplicate gaps):',
    clip(lines.join('\n'), 18000),
  ].filter(Boolean).join('\n');
...
async function summarizeCommit(cfg, { patch, files, branch, truncated }) {
  if (!cfg.model) bad('Pick a model first');
...
module.exports = {
  status, classify, suggest, suggestMilestones, planInsights, summarizeCommit,
  embed, cosine, topK, issueText, unload, DEFAULT_ENDPOINT,
};
