...
function run(cmd, args, opts = {}) {
  /*
   * `binary: true` keeps stdout as a Buffer.
   *
   * The default path decodes stdout as UTF-8, which is right for every text answer git gives
   * and silently destroys every byte answer: any sequence that is not valid UTF-8 becomes
   * U+FFFD, so `git show :2:sprite.png` comes back as a corrupted string that merely looks
   * like data. Anything reading blob CONTENT rather than blob text has to ask for this.
   */
  ...
  return new Promise((resolve, reject) => {
    execFile(cmd, args, {
      ...
      const out = binary ? (stdout || Buffer.alloc(0)) : String(stdout || '');
      const errOut = binary ? String(stderr || '') : String(stderr || '');
      if (err) {
        let msg = errOut.trim() || (binary ? '' : out.trim()) || String(err.message || '').trim();
...
/* The same, for blob CONTENT rather than blob text. Reads only — there is no binary write. */
const gitBytes = (dir, args) => run('git', args, { cwd: dir, binary: true }).then(r => r.stdout);
...
module.exports = {
  run, git, gitBytes, gh, gitWrite, ghWrite, mutate,
