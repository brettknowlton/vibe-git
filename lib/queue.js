...
const KINDS = {
  ...
  /*
   * Changing a milestone that already exists — the counterpart to creating one.
   *
   * The whole Plan view is built on milestone titles, descriptions and due dates: they
   * decide the phase bands, the ordering, and what the model is told each milestone is FOR.
   * Being able to create one but never correct it meant every one of those inputs was
   * editable only on github.com, which is the one place this app exists to save you from.
   *
   * Addressed by milestone NUMBER, not by title, because a rename is the commonest edit and
   * an operation keyed on the thing being changed cannot survive it.
   *
   * `dueOn: null` clears the date rather than leaving it alone — "no deadline" is a real
   * answer and there has to be a way to say it. An omitted key means "unchanged"; a null
   * means "remove". Those are different and the queue must not collapse them.
   */
  milestoneEdit: {
    validate(p, ctx) {
      const number = posInt(p.number, 'milestone number');
      const known = ctx && ctx.milestones && ctx.milestones.find(m => m.number === number);
      const out = { number, was: known ? known.title : null };
      let touched = false;
      if (p.title != null) {
        out.title = noLeadingDash(text(p.title, 'milestone title', 200), 'milestone title');
        if (ctx && ctx.milestones && ctx.milestones.some(m => m.title === out.title && m.number !== number)) {
          bad(`Another milestone is already called “${out.title}”`);
        }
        touched = true;
      }
      if (p.description != null) {
        out.description = text(p.description, 'description', 4000, { required: false }) || '';
        touched = true;
      }
      if (p.dueOn !== undefined) {
        if (p.dueOn === null || p.dueOn === '') out.dueOn = null;
        else {
          const d = String(p.dueOn).trim();
          if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) bad('Due date must be YYYY-MM-DD');
          if (isNaN(new Date(d + 'T00:00:00Z').getTime())) bad('That is not a real date');
          out.dueOn = d;
        }
        touched = true;
      }
      if (p.state != null) {
        const s = text(p.state, 'state', 10);
        if (s !== 'open' && s !== 'closed') bad('Milestone state must be "open" or "closed"');
        out.state = s;
        touched = true;
      }
      if (!touched) bad('That milestone edit changes nothing');
      return out;
    },
    describe(p) {
      const bits = [];
      if (p.title != null) bits.push(`rename to “${p.title}”`);
      if (p.description != null) bits.push('new description');
      if (p.dueOn !== undefined) bits.push(p.dueOn ? `due ${p.dueOn}` : 'clear the due date');
      if (p.state) bits.push(p.state === 'closed' ? 'close it' : 'reopen it');
      return `Milestone ${p.was ? `“${p.was}”` : '#' + p.number}: ` + bits.join(', ');
    },
    argv(p) {
      const a = ['api', '--method', 'PATCH', 'repos/:owner/:repo/milestones/' + p.number];
      if (p.title != null) a.push('-f', 'title=' + p.title);
      if (p.description != null) a.push('-f', 'description=' + p.description);
      // -F sends a typed value, which is how a null reaches the API as JSON null rather
      // than as the four-character string "null" — the difference between clearing the due
      // date and setting it to something unparseable.
      if (p.dueOn !== undefined) {
        if (p.dueOn === null) a.push('-F', 'due_on=null');
        else a.push('-f', 'due_on=' + p.dueOn + 'T00:00:00Z');
      }
      if (p.state) a.push('-f', 'state=' + p.state);
      return a;
    },
  },
...
class Queue {
  ...
  async push(repoPath, dir) {
    ...
    // so they go first regardless of the order they were staged in. A milestone RENAME
    // belongs in the same group for the same reason pointing the other way: issue edits
    // name a milestone by title, so a rename staged after them would leave those edits
    // referring to a title that no longer exists.
    const first = (c) => c.kind === 'milestone' || c.kind === 'label' || c.kind === 'milestoneEdit';
