...
/*
 * Muting marks, it does not delete.
 *
 * A muted entry stays in the plan and keeps its rank; it is flagged so the view can fold it
 * away behind a toggle. Dropping it here instead would renumber everything below it and make
 * "hide this for now" indistinguishable from "this was never recommended", which is the one
 * thing a reader must be able to tell apart before acting on the order.
 */
function applyMutes(plan, muted) {
  const sets = mutes.keySets(muted);
  if (!sets.items.size && !sets.deps.size) return plan;
  return Object.assign({}, plan, {
    ranked: (plan.ranked || []).map(item => {
      const key = mutes.itemKey(item);
      return key && sets.items.has(key) ? Object.assign({}, item, { muted: true, muteKey: key }) : item;
    }),
    /* Per-edge, so rejecting one wrong blocker out of three keeps the other two offerable. */
    deps: (plan.deps || []).map(dep => {
      const hidden = (dep.blockedBy || []).filter(n => sets.deps.has(mutes.depKey(dep.blocked, n)));
      if (!hidden.length) return dep;
      return Object.assign({}, dep, {
        mutedBy: hidden,
        muted: hidden.length === (dep.blockedBy || []).length,
      });
    }),
  });
}

function hydratePlan(raw, milestones, issues, muted) {
  if (!raw || typeof raw !== 'object') return null;
  ...
  return applyMutes(Object.assign({}, raw, {
...
function programmaticPlan(repo, milestones, issues, count = 10, muted) {
  const raw = {
    schemaVersion: 2,
    repo,
  ...
  return hydratePlan(raw, milestones, issues, muted);
}

/*
 * A milestone is "done" when GitHub says it is closed, or when it holds issues and none of
 * them are still open. The second half matters because closing the milestone itself is a
 * step people skip for months, and a plan header whose first band is a finished phase reports
 * a due date that passed and a countdown of zero — which reads as an emergency rather than as
 * work that is already behind you.
 *
 * A milestone with no issues at all is NOT done. It is empty, which is a different thing and
 * usually means the work has not been filed yet.
 */
function phaseComplete(phase, issues) {
  if (!phase) return false;
  if (String(phase.state || '').toLowerCase() === 'closed') return true;
  const mine = (issues || []).filter(issue => issue && issue.p === phase.n);
  return mine.length > 0 && mine.every(issue => issue.st !== 'OPEN');
}
...
module.exports = {
  ...
  normalizeScope, inScope, scopeText, phaseComplete, applyMutes,
