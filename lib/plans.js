...
function normalizeGeneratedGaps(rawGaps, {
  ...
  for (const raw of rawGaps || []) {
    ...
    const milestone = milestones.find(item => item.title === raw.milestone) || null;
    const phase = milestone ? phaseForMilestone(milestone.title, phases) : null;
    const label = (raw.labels || []).find(item => allowedLabels.has(item)) || null;
...
function hydratePlan(raw, milestones, issues) {
  ...
  const ordered = prioritizeIssues(phasedIssues, milestones, phases);
