...
const TOOLS = [
  ...
  fn('propose_dependency',
    'Declare that one issue is blocked by another. Writes "Blocked by: #N" into the blocked issue\'s body, which is how this app records dependencies. Review only — nothing is written until the user pushes. Read both issues first.',
    {
      blocked: { type: 'number', description: 'The issue that has to wait.' },
      blocked_by: { type: 'array', items: { type: 'number' }, description: 'The issue numbers it is waiting on.' },
...
async function chat(cfg, { messages, ctx, signal, maxSteps }) {
  ...
  for (let step = 0; step < limit; step++) {
    ...
    for (const call of turn.calls) {
      if (signal) signal.step = stepText(call.name, call.args);
