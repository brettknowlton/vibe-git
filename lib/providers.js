...
/*
 * How long a local model stays resident between requests.
 *
 * Ollama's own default is five minutes, and five minutes is shorter than the gap between
 * reading a plan and asking the assistant about it — so the common rhythm of using this app
 * pays a full model load on almost every action. On a 26GB MoE that is twenty to forty
 * seconds of nothing happening before the first token, repeatedly, for no reason other than
 * an idle timer. Holding the model is the single largest latency win available here and it
 * costs only VRAM that nothing else is asking for.
 *
 * `null` means "say nothing and let the server decide", which is what a hosted endpoint and
 * anyone who manages residency themselves want. A negative number means indefinitely.
 */
function keepAliveValue(cfg) {
  const raw = cfg && cfg.keepAliveMinutes;
  if (raw == null || raw === '') return null;
  const minutes = Number(raw);
  if (!Number.isFinite(minutes)) return null;
  // Zero is "say nothing", NOT keep_alive: 0 — which Ollama reads as "unload the moment this
  // request finishes", the exact opposite of leaving the server's own policy alone.
  if (minutes === 0) return null;
  if (minutes < 0) return -1;                 // Ollama reads any negative as "forever"
...
const ollama = {
  ...
  async chat(cfg, messages, { schema = null, tools = null, numCtx = null, signal = null } = {}) {
    const payload = {
      ...
      options: {
        ...
        /*
         * Without a ceiling a model that fails to close its JSON generates until the context
         * is full, which on a 32k window is minutes of tokens nobody will ever read followed
         * by a parse error. The same half of the window the OpenAI dialect asks for below.
         */
        num_predict: Math.max(1024, Math.min(Math.round((numCtx || cfg.numCtx || 8192) / 2), 16384)),
      },
    };
    const keepAlive = keepAliveValue(cfg);
  ...
  async embed(cfg, input, signal) {
    // The embedder is reloaded far more often than the chat model — every semantic lookup
    // touches it — and it is small enough that holding it costs almost nothing.
    const keepAlive = keepAliveValue(cfg);
    const resident = (payload) => (keepAlive == null ? payload : Object.assign({ keep_alive: keepAlive }, payload));
    ...
    try {
      const r = await requestJson('POST', apiUrl(cfg.endpoint, 'api/embed'),
        { payload: resident({ model: cfg.embedModel, input }), timeoutMs: cfg.timeoutMs || 300000, signal });
    ...
    for (const t of input) {
      const r = await requestJson('POST', apiUrl(cfg.endpoint, 'api/embeddings'),
        { payload: resident({ model: cfg.embedModel, prompt: t }), timeoutMs: cfg.timeoutMs || 300000, signal });
...
module.exports = {
  ...
  redact, resolveKey, hasKey, apiUrl, requestJson, withCallIds, keepAliveValue,
  providers: BY_ID,
};
